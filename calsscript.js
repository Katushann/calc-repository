const result = document.querySelector('#result'),
      ...
      bs = document.querySelector('#delete1smb');
      ms = document.querySelector('#savenumber');
      mr = document.querySelector('#shownumber');
        mc = document.querySelector('#clearmemory');
...
bs.addEventListener('click', () => {
  if (!expression.innerHTML.match(/=$/)) {
    ...
    ex = doCE(ex); 
      ...
      function doCE(arg) {
          result.innerHTML = arg.slice(0, arg.length - 1);
        return arg.slice(0, arg.length - 1);
    }
  }
})
// save the last number on click
ms.addEventListener('click', () => {
  ...
  let ar = ex.split(/\/|\*|\+|-|=/);
    ...
    {
        memory = ar[ar.length - 1]
    }
   })
   // show the number saved
